...
from project_mine.src.datasets.process_mols import parse_pdb_from_path, generate_conformer, read_molecule, get_lig_graph_with_matching, \
...
def get_sequences_from_pdbfile(file_path):
	...
	structure = structure[0] # type: ignore
	sequence = None
	for i, chain in enumerate(structure):
		...
		for res_idx, residue in enumerate(chain):
			...
			for atom in residue:
				if atom.name == 'CA':
					c_alpha = list(atom.get_vector()) # type: ignore
				if atom.name == 'N':
					n = list(atom.get_vector()) # type: ignore
				if atom.name == 'C':
					c = list(atom.get_vector()) # type: ignore
...
def compute_ESM_embeddings(model, alphabet, labels, sequences):
	...
	data_loader = torch.utils.data.DataLoader(
		dataset, collate_fn=alphabet.get_batch_converter(truncation_seq_length), batch_sampler=batches # type: ignore
	)
...
class InferenceDataset(Dataset):
	def __init__(self, out_dir, complex_names, protein_files, ligand_descriptions, protein_sequences, lm_embeddings,
		...
		self.p2rank_prior = pd.read_csv('data/p2rank/results.csv')
	...
	def get(self, idx):
		name, protein_file, ligand_description, lm_embedding = \
			self.complex_names[idx], self.protein_files[idx], self.ligand_descriptions[idx], self.lm_embeddings[idx] # type: ignore
		data_path = f"data/inf_data/{name}_complex_aa{self.all_atoms}.pt"
		if os.path.exists(data_path):
			complex_graph = torch.load(data_path, weights_only=False)
			...
			try:
				...
				if mol is not None:	# F
					...
					GT_priot = mol.GetConformer().GetPositions().mean(axis=0) # type: ignore
					if mol is None:
			...
			####### Prior
			prior = self.p2rank_prior[self.p2rank_prior['pdb_id'] == name]
			prior = prior[['x', 'y', 'z']].values
			if len(prior) != 0:
				prior = torch.from_numpy(prior).float() - protein_center
				complex_graph.ligand_center = prior
			else:
				complex_graph.ligand_center = torch.tensor([0, 0, 0], dtype=torch.float)
