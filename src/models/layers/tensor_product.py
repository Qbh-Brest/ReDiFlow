...
class FasterTensorProduct(torch.nn.Module):
    ...
    def forward(self, in_, sh, weight):
        ...
        for (m, ir), sl in zip(self.in_irreps, self.in_irreps.slices()): # type: ignore
        ...
        if out_dict["0e"]:
            out_dict["0e"] = torch.cat(out_dict["0e"], dim=-1) # type: ignore
            out_dict["0e"] = torch.matmul( # type: ignore
                out_dict["0e"].unsqueeze(-2), weight_dict["0e"]
            ).squeeze(-2)

        if out_dict["1o"]:
            out_dict["1o"] = torch.cat(out_dict["1o"], dim=-2) # type: ignore
            out_dict["1o"] = (
        ...
        if out_dict["1e"]:
            out_dict["1e"] = torch.cat(out_dict["1e"], dim=-2) # type: ignore
            out_dict["1e"] = (
        ...
        if out_dict["0o"]:
            out_dict["0o"] = torch.cat(out_dict["0o"], dim=-1) # type: ignore
            # out_dict['0o'] = (out_dict['0o'].unsqueeze(-1) * weight_dict['0o']).sum(-2)
            out_dict["0o"] = torch.matmul( # type: ignore
...
class TensorProductConvLayer(torch.nn.Module):
    def __init__(
        ...
        norm_type: Optional[Literal["batch_norm", "layer_norm"]] = 'batch_norm',
    ):
        super().__init__()
        ...
        if faster:
            log.info("Faster Tensor Product")
        ...
        self.norm = get_norm_layer(
            out_irreps=out_irreps, norm_type=norm_type, affine=norm_affine # type: ignore
        )
    ...
    def forward(
        ...
        out_nodes: int = None,  # type: ignore
        reduce: str = "mean",
